// FriendsListCtrl.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(CFriendsListCtrl, CListCtrl)
	...
	ON_COMMAND(IDM_VIDEOCHAT, OnVideochat)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CFriendsListCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	if( -1 != m_nCamera )
	{
		m_pMainDlg->OnVideoChat( m_nCamera );
		m_nCamera = -1;
	}
	else if( -1 != m_nDelIcon )
	{
...
void CFriendsListCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	/// 判断摄像头是否有焦点
	if( pUser->bHasCamera && m_nCamera != nIndex && rect.PtInRect( point ) )
	{
		m_nCamera	= nIndex;
		SetItemState( nIndex, ~LVIS_FOCUSED, LVIS_FOCUSED );
	}
	else if( -1 != m_nCamera && !rect.PtInRect( point ) )
	{
		m_nCamera = -1;
		SetItemState( nIndex, ~LVIS_FOCUSED, LVIS_FOCUSED );
	}
...
void CFriendsListCtrl::OnVideochat() 
{
	if( -1 == m_nCurSel )
	{
		return ;
	}
	m_pMainDlg->OnVideoChat( m_nCurSel );
}
