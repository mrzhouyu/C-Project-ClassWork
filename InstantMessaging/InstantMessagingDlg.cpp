...
void CInstantMessagingDlg::OnListeningReceive()
{
	...
	case SENDFILES_DENY:											/// 拒绝接收文件
		...
		break;
	case SHARESCREEN_ADDREQUEST:									/// 共享屏幕添加请求
		memcpy( &user, szReceive + sizeof( DATAPACKET ), sizeof( USER ) );
		memcpy( user.strIP, strFriendIP.GetBuffer( 16 ), 16 );
		strFriendIP.ReleaseBuffer();
	...
	case WHITEBOARD_ADDREQUEST:										/// 白板添加请求
		...
		break;
	case VIDEOCHAT_VIDEO_ADDREQUEST:								/// 视频请求
		memcpy( &user, szReceive + sizeof( DATAPACKET ), sizeof( USER ) );
		memcpy( user.strIP, strFriendIP.GetBuffer( 16 ), 16 );
		strFriendIP.ReleaseBuffer();
		OnAddToVideoChatRequest( user );
		break;
	case VIDEOCHAT_VIDEO_ADDREQUEST_DENY:							/// 拒绝视频聊天
		memcpy( &user, szReceive + sizeof( DATAPACKET ), sizeof( USER ) );
		memcpy( user.strIP, strFriendIP.GetBuffer( 16 ), 16 );
		strFriendIP.ReleaseBuffer();
		DenyAddToVideoChatRequest( user );
		break;
	case VIDEOCHAT_VIDEO_ADDREQUEST_ALLOW:							/// 允许视频聊天
		memcpy( &user, szReceive + sizeof( DATAPACKET ), sizeof( USER ) );
		memcpy( user.strIP, strFriendIP.GetBuffer( 16 ), 16 );
		strFriendIP.ReleaseBuffer();
		AllowAddToVideoChatRequest( user );
		break;
	case VIDEOCHAT_VIDEO_DISCONNECT:								/// 断开视频连接
		memcpy( &user, szReceive + sizeof( DATAPACKET ), sizeof( USER ) );
		memcpy( user.strIP, strFriendIP.GetBuffer( 16 ), 16 );
		strFriendIP.ReleaseBuffer();
...
void CInstantMessagingDlg::OnAddToShareScreenRequest( const USER userServer )
{
	...
	/// 提示是否加入共享屏幕
	CString strPrompt;
	strPrompt.Format( "来自 %s 的好友 %s 请求分享Ta的屏幕\n您同意吗？", userServer.strIP, userServer.strName );
...
void CInstantMessagingDlg::OnAddToVideoChatRequest( const USER user )
{
	...
	/// 提示是否加入视频聊天
	CString strPrompt;
	strPrompt.Format( "来自 %s 的好友 %s 请求您视频聊天\n您同意吗？", user.strIP, user.strName );

	/// 拒绝请求
	if( IDCANCEL == MessageBox( strPrompt, "视频聊天", MB_OKCANCEL | MB_DEFBUTTON1 | MB_ICONQUESTION ) )
	{
		/// 向服务器发送拒绝加入视频聊天的请求
...
void CInstantMessagingDlg::CloseVideoChatDlg( const USER user )
{
	...
	/// 如果没有视频聊天对话框，则关闭发送和接收线程并断开和视频的连接
